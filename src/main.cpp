...
#if defined(NDEBUG)
# error "btcrcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb67264e88dc1f015a5daee85a55ea0ee80e46897ad1181a783e8169810a7982e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // btcrcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "btcrcoin Signed Message:\n";
...
{
    // btcrcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // btcrcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // btcrcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // btcrcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb67264e88dc1f015a5daee85a55ea0ee80e46897ad1181a783e8169810a7982e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Just another coin based on litecoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x7c7d8fdc28c3e71e4c9773e29d174105ca942fcaf776268ccdcdf5617ba2203c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // btcrcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("btcrcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("btcrcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static btcrcoinMiner(CWallet *pwallet)
{
    printf("btcrcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btcrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running btcrcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("btcrcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&btcrcoinMiner, pwallet));
}
